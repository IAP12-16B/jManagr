...
public class UsersTest
{
	...
	{
		testUser = new User("Test_User", "Testi", "T.Test", "123", USER_ROLE.USER, true, false);
	...
	public void testValidateUserExists() throws Exception
	{
		User testUser2 = new User(
				testUser.getFirstname(), testUser.getLastname(), testUser.getUsername(),
				testUser.getPassword(), testUser.getRole(), true, false
		);
		assertEquals(users.validate(testUser2), STATUS_CODE.ALREADY_EXISTS);

		User testUser3 = new User("A", "B", "C", "D", USER_ROLE.ADMIN, true, false);
		assertEquals(users.validate(testUser3), STATUS_CODE.OK);

		User testUser4 = new User("A", "B", testUser.getUsername(), "D", USER_ROLE.ADMIN, true, false);
	...
	{
		User testUser2 = new User("|&%~`", "B", "T", "D", USER_ROLE.AGENT, true, false);
		assertEquals(STATUS_CODE.NAME_INVALID, users.validate(testUser2));

		User testUser3 = new User("A", "B", "¬&@?`", "D", USER_ROLE.AGENT, true, false);
		assertEquals(STATUS_CODE.USERNAME_INVALID, users.validate(testUser3));

		User testUser4 = new User("A", "B", "", "D", USER_ROLE.AGENT, true, false);
		assertEquals(STATUS_CODE.USERNAME_INVALID, users.validate(testUser4));

		User testUser5 = new User("A", "B", "C", "", USER_ROLE.AGENT, true, false);
		assertEquals(STATUS_CODE.PASSWORD_INVALID, users.validate(testUser5));

		User testUser6 = new User("A", "B", "C", "123", USER_ROLE.AGENT, true, false);
		assertEquals(STATUS_CODE.PASSWORD_INVALID, users.validate(testUser6));

		User testUser7 = new User("A", "B", "C", "Hello123imagoodpassword", USER_ROLE.AGENT, true, false);
